...
std::vector<float> ReadWave(const unsigned char* wave_data, size_t wave_size,
                            float expected_sample_rate, bool *is_ok) {
  std::istringstream is(std::string(reinterpret_cast<const char*>(wave_data), wave_size));
  return ReadWave(is, expected_sample_rate, is_ok);
}
