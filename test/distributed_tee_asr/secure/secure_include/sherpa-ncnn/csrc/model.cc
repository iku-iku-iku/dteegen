...
void Model::InitNet(ncnn::Net &net, const unsigned char *param_data,
                    const unsigned char *bin_data) {
  net.load_param(param_data);

  net.load_model(bin_data);
}
...
std::unique_ptr<Model> Model::Create(const ModelConfig &config) {
  ...
  auto ret = net.load_param(config.encoder_param_data);
  // if (ret != 0) {
