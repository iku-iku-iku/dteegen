...
SymbolTable::SymbolTable(const unsigned char* tokens_data, size_t tokens_size) {
  std::istringstream is(std::string(reinterpret_cast<const char*>(tokens_data), tokens_size));
  Init(is);
}
