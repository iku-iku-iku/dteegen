...
#include "ncnn_retinanet/retinanet.h"
#include "stb_image.h"
...
int main(int argc, char **argv) {
  ...
  auto ctx = init_distributed_tee_context({.side = SIDE::Client,
                                           ...
                                           .version = "1.0"});
  if (*record) {
    std::cout << "Recording: " << img_path << " with person ID: " << person_id
              << std::endl;
    auto image_data = detect_face_and_load(img_path.c_str());
    std::array<char, IMG_SIZE> arr;
    memcpy(arr.data(), image_data, IMG_SIZE);
    int res = call_remote_secure_function(ctx, img_recorder, arr, person_id);
