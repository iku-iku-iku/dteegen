...
public:
    BitmapMemoryPool(void* memPool, size_t poolSize, size_t blockSize) 
        ...
        bitmap.resize((numBlocks + 7) / 8, 0); // 初始化位图，所有位都设置为0（空闲）
    }

    virtual ~BitmapMemoryPool() override {}

    virtual void* fastMalloc(size_t size) override {
        size_t numBlocksNeeded = (size + blockSize - 1) / blockSize; // 计算所需的块数
    ...
    virtual void fastFree(void* ptr) override {
        ...
        size_t blockIndex = (static_cast<char*>(ptr) - static_cast<char*>(pool)) / blockSize;
...
int embedding(in_char img[IMG_SIZE], out_char res[EMBEDDING_SIZE]) {
  ...
  // if (net.load_param(ncnn::DataReaderFromMemory(mobilefacenet_param_ptr)))
  //   exit(-1);
  // if (net.load_model(ncnn::DataReaderFromMemory(mobilefacenet_bin_ptr)))
  //   exit(-1);

  ncnn::Mat input =
      ncnn::Mat::from_pixels((const unsigned char*)img, ncnn::Mat::PIXEL_RGB, WIDTH, HEIGHT);
